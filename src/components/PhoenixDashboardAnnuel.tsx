...
export default function PhoenixDashboardAnnuel() {
  ...
  const victoires = matchs?.filter(m => m.buts_marques > m.buts_encaisses).length || 0;
  const defaites = matchs?.filter(m => m.buts_marques < m.buts_encaisses).length || 0;
  const nuls = matchs?.filter(m => m.buts_marques === m.buts_encaisses).length || 0;
    ...
    <div className="space-y-6">
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <Card>
          ...
          <CardContent>
            <p className="text-4xl font-bold">
              {matchs?.reduce((sum, m) => sum + (m.buts_marques || 0), 0) || 0}
            </p>
            <p className="text-sm text-muted-foreground mt-2">
              Moyenne: {totalMatchs > 0 
                ? (matchs?.reduce((sum, m) => sum + (m.buts_marques || 0), 0) / totalMatchs).toFixed(1) 
                : 0} par match
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <p className="text-4xl font-bold">
              {matchs?.reduce((sum, m) => sum + (m.buts_encaisses || 0), 0) || 0}
            </p>
            <p className="text-sm text-muted-foreground mt-2">
              Moyenne: {totalMatchs > 0 
                ? (matchs?.reduce((sum, m) => sum + (m.buts_encaisses || 0), 0) / totalMatchs).toFixed(1) 
                : 0} par match
            </p>
          </CardContent>
        </Card>
