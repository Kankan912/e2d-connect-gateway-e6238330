import { useE2DButeursClassement } from "@/hooks/useE2DPlayerStats";
...
export default function E2DClassementButeurs() {
  const { data: buteurs, isLoading } = useE2DButeursClassement();
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          <Trophy className="h-5 w-5 text-yellow-500" />
          Classement des Buteurs E2D
        </CardTitle>
      </CardHeader>
      <CardContent>
        {!buteurs || buteurs.length === 0 ? (
          <div className="text-center py-8 text-muted-foreground">
            <Target className="h-12 w-12 mx-auto mb-2 opacity-50" />
            <p>Aucun but enregistré pour le moment</p>
          </div>
        ) : (
          <Table>
            <TableHeader>
