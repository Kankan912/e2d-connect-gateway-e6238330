...
export default function CompteRenduViewer({ open, onOpenChange, reunion, onEdit }: CompteRenduViewerProps) {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      const presents = presences?.filter((p: { statut_presence?: string | null; heure_arrivee?: string | null; observations?: string | null; membres?: { nom?: string; prenom?: string } | null }) => p.statut_presence === 'present') || [];
      const excuses = presences?.filter((p: { statut_presence?: string | null; heure_arrivee?: string | null; observations?: string | null; membres?: { nom?: string; prenom?: string } | null }) => p.statut_presence === 'absent_excuse') || [];
      const absentsNonExcuses = presences?.filter((p: { statut_presence?: string | null; heure_arrivee?: string | null; observations?: string | null; membres?: { nom?: string; prenom?: string } | null }) => p.statut_presence === 'absent_non_excuse') || [];
      const retards = presences?.filter((p: { statut_presence?: string | null; heure_arrivee?: string | null; observations?: string | null; membres?: { nom?: string; prenom?: string } | null }) => p.heure_arrivee) || [];
      ...
      if (presents.length > 0) {
        const presentsText = presents
          .map((p: { statut_presence?: string | null; heure_arrivee?: string | null; observations?: string | null; membres?: { nom?: string; prenom?: string } | null }) => `${p.membres?.prenom} ${p.membres?.nom}`)
          .join(', ');
      ...
      if (excuses.length > 0) {
        ...
        const excusesText = `Excusés: ${excuses.map((p: { statut_presence?: string | null; heure_arrivee?: string | null; observations?: string | null; membres?: { nom?: string; prenom?: string } | null }) => `${p.membres?.prenom} ${p.membres?.nom}`).join(', ')}`;
      ...
      if (retards.length > 0) {
        doc.setFont('helvetica', 'italic');
        const retardsText = `Retards: ${retards.map((p: { statut_presence?: string | null; heure_arrivee?: string | null; observations?: string | null; membres?: { nom?: string; prenom?: string } | null }) => `${p.membres?.prenom} ${p.membres?.nom}`).join(', ')}`;
      ...
      if (absentsNonExcuses.length > 0) {
        ...
        const absentsText = `Absents non excusés: ${absentsNonExcuses.map((p: { statut_presence?: string | null; heure_arrivee?: string | null; observations?: string | null; membres?: { nom?: string; prenom?: string } | null }) => `${p.membres?.prenom} ${p.membres?.nom}`).join(', ')}`;
      ...
      if (comptesRendus && comptesRendus.length > 0) {
        comptesRendus.forEach((cr: { id?: string; titre?: string | null; description?: string | null; numero_ordre?: number | null; decisions?: string | null }, index: number) => {
          checkNewPage();
      ...
      if (cotisationsReunion && cotisationsReunion.length > 0) {
        checkNewPage();
        const totalCotisations = cotisationsReunion.reduce((sum: number, c: { montant?: number | null; membre?: { nom?: string; prenom?: string } | null; type?: { nom?: string } | null }) => sum + (c.montant || 0), 0);
        doc.setFont('helvetica', 'bold');
        doc.setFontSize(12);
        ...
        doc.setFont('helvetica', 'normal');
        doc.setFontSize(10);
        cotisationsReunion.forEach((c: { montant?: number | null; membre?: { nom?: string; prenom?: string } | null; type?: { nom?: string } | null }) => {
          checkNewPage(10);
      ...
      if (epargnesReunion && epargnesReunion.length > 0) {
        checkNewPage();
        const totalEpargnes = epargnesReunion.reduce((sum: number, e: { montant?: number | null; membre?: { nom?: string; prenom?: string } | null }) => sum + (e.montant || 0), 0);
        doc.setFont('helvetica', 'bold');
        doc.setFontSize(12);
        ...
        doc.setFont('helvetica', 'normal');
        doc.setFontSize(10);
        epargnesReunion.forEach((e: { montant?: number | null; membre?: { nom?: string; prenom?: string } | null }) => {
          checkNewPage(10);
      ...
      if (sanctionsReunion && sanctionsReunion.length > 0) {
        checkNewPage();
        const totalSanctions = sanctionsReunion.reduce((sum: number, s: { montant_amende?: number | null; type_sanction?: string | null; membre?: { nom?: string; prenom?: string } | null }) => sum + (s.montant_amende || 0), 0);
        doc.setFont('helvetica', 'bold');
        doc.setFontSize(12);
        ...
        doc.setFont('helvetica', 'normal');
        doc.setFontSize(10);
        sanctionsReunion.forEach((s: { montant_amende?: number | null; type_sanction?: string | null; membre?: { nom?: string; prenom?: string } | null }) => {
          checkNewPage(10);
      ...
      if (aidesReunion && aidesReunion.length > 0) {
        checkNewPage();
        const totalAides = aidesReunion.reduce((sum: number, a: { montant?: number | null; type_aide?: string | null; membre?: { nom?: string; prenom?: string } | null }) => sum + (a.montant || 0), 0);
        doc.setFont('helvetica', 'bold');
        doc.setFontSize(12);
        ...
        doc.setFont('helvetica', 'normal');
        doc.setFontSize(10);
        aidesReunion.forEach((a: { montant?: number | null; type_aide?: string | null; membre?: { nom?: string; prenom?: string } | null }) => {
          checkNewPage(10);
      ...
      if (beneficiairesReunion && beneficiairesReunion.length > 0) {
        checkNewPage();
        const totalBeneficiaires = beneficiairesReunion.reduce((sum: number, b: { montant_final?: number | null; membre?: { nom?: string; prenom?: string } | null }) => sum + (b.montant_final || 0), 0);
        doc.setFont('helvetica', 'bold');
        doc.setFontSize(12);
        ...
        doc.setFont('helvetica', 'normal');
        doc.setFontSize(10);
        beneficiairesReunion.forEach((b: { montant_final?: number | null; membre?: { nom?: string; prenom?: string } | null }) => {
          checkNewPage(10);
  ...
  const presentsCount = presences?.filter((p: { statut_presence?: string | null; heure_arrivee?: string | null; observations?: string | null; membres?: { nom?: string; prenom?: string } | null }) => p.statut_presence === 'present').length || 0;
  const excusesCount = presences?.filter((p: { statut_presence?: string | null; heure_arrivee?: string | null; observations?: string | null; membres?: { nom?: string; prenom?: string } | null }) => p.statut_presence === 'absent_excuse').length || 0;
  const absentsNonExcusesCount = presences?.filter((p: { statut_presence?: string | null; heure_arrivee?: string | null; observations?: string | null; membres?: { nom?: string; prenom?: string } | null }) => p.statut_presence === 'absent_non_excuse').length || 0;
  ...
  const totalCotisations = cotisationsReunion?.reduce((sum: number, c: { montant?: number | null; membre?: { nom?: string; prenom?: string } | null; type?: { nom?: string } | null }) => sum + (c.montant || 0), 0) || 0;
  const totalEpargnes = epargnesReunion?.reduce((sum: number, e: { montant?: number | null; membre?: { nom?: string; prenom?: string } | null }) => sum + (e.montant || 0), 0) || 0;
  const totalSanctions = sanctionsReunion?.reduce((sum: number, s: { montant_amende?: number | null; type_sanction?: string | null; membre?: { nom?: string; prenom?: string } | null }) => sum + (s.montant_amende || 0), 0) || 0;
  const totalAides = aidesReunion?.reduce((sum: number, a: { montant?: number | null; type_aide?: string | null; membre?: { nom?: string; prenom?: string } | null }) => sum + (a.montant || 0), 0) || 0;
  const totalBeneficiaires = beneficiairesReunion?.reduce((sum: number, b: { montant_final?: number | null; membre?: { nom?: string; prenom?: string } | null }) => sum + (b.montant_final || 0), 0) || 0;
          ...
          <div className="space-y-4 pr-4">
              ...
              <Card>
                ...
                <CardContent>
                  <div className="grid grid-cols-2 gap-2">
                    {presences?.filter((p: { statut_presence?: string | null; heure_arrivee?: string | null; observations?: string | null; membres?: { nom?: string; prenom?: string } | null }) => p.statut_presence === 'present').map((presence: { statut_presence?: string | null; heure_arrivee?: string | null; observations?: string | null; membres?: { nom?: string; prenom?: string } | null }, index: number) => (
                      <div
                        key={index}
            ...
            {absentsNonExcusesCount > 0 && (
              <Card className="border-destructive/50">
                ...
                <CardContent>
                  <div className="grid grid-cols-2 gap-2">
                    {presences?.filter((p: { statut_presence?: string | null; heure_arrivee?: string | null; observations?: string | null; membres?: { nom?: string; prenom?: string } | null }) => p.statut_presence === 'absent_non_excuse').map((presence: { statut_presence?: string | null; heure_arrivee?: string | null; observations?: string | null; membres?: { nom?: string; prenom?: string } | null }, index: number) => (
                      <div
                        key={index}
              ...
              <CardContent>
                {isLoading ? (
                  ...
                  <div className="space-y-4">
                    {comptesRendus?.map((cr: { id?: string; titre?: string | null; description?: string | null; numero_ordre?: number | null; decisions?: string | null }) => (
                      <div
                        key={cr.id}
              ...
              <Card>
                ...
                <CardContent>
                  <div className="space-y-2">
                    {cotisationsReunion?.map((c: { montant?: number | null; membre?: { nom?: string; prenom?: string } | null; type?: { nom?: string } | null }) => (
            ...
            {epargnesCount > 0 && (
              <Card>
                ...
                <CardContent>
                  <div className="space-y-2">
                    {epargnesReunion?.map((e: { montant?: number | null; membre?: { nom?: string; prenom?: string } | null }) => (
            ...
            {sanctionsCount > 0 && (
              <Card className="border-orange-200 dark:border-orange-800">
                ...
                <CardContent>
                  <div className="space-y-2">
                    {sanctionsReunion?.map((s: { montant_amende?: number | null; type_sanction?: string | null; membre?: { nom?: string; prenom?: string } | null }) => (
            ...
            {aidesCount > 0 && (
              <Card className="border-green-200 dark:border-green-800">
                ...
                <CardContent>
                  <div className="space-y-2">
                    {aidesReunion?.map((a: { montant?: number | null; type_aide?: string | null; membre?: { nom?: string; prenom?: string } | null }) => (
            ...
            {beneficiairesCount > 0 && (
              <Card className="border-primary/50">
                ...
                <CardContent>
                  <div className="space-y-2">
                    {beneficiairesReunion?.map((b: { montant_final?: number | null; membre?: { nom?: string; prenom?: string } | null }) => (
